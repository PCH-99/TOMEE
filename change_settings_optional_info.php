...
function filter_data($x,$old_x,$connect)
{
	if($x == '' || $x == ' ') $x = $old_x;
	else
	{
		$x = trim(stripslashes(htmlspecialchars($x, ENT_QUOTES)));
...
if(!isset($_SESSION['logged']) || !isset($_POST['name'])) 
{
	...
	try
	{
		...
		else
		{
			...
			else
			{
				...
				$name = filter_data($_POST['name'],$result['name'],$connect);
				$surname = filter_data($_POST['surname'],$result['surname'],$connect);
				$gender = filter_data($_POST['gender'],$result['gender'],$connect);
				
				if($gender == 'kobieta' || $gender == 'mężczyzna' || $gender == 'Nie podano')
				{
					$age = $_POST['age'];
					if($age == '' || $age == ' ') $age = $result['age'];
					else
					{
						if(verify_id($age) == false) $_SESSION['no_settings'] = 'nie podano wieku w liczbach!';
						else
						{
							if($age > 120) $_SESSION['no_settings'] = 'według podanego wieku jest niewiarygodnie stary/a !!!';
						}
					}
				}
				else $_SESSION['no_settings'] = 'nie określiłeś swojej płci';
				
				if(!isset($_SESSION['no_settings']))
				{
					$job = filter_data($_POST['job'],$result['job'],$connect);
					$country = filter_data($_POST['country'],$result['country'],$connect);
					$l_p = filter_data($_POST['l_p'],$result['live_place'],$connect);
					$m_p = filter_data($_POST['m_s'],$result['marital_status'],$connect);
					
					$query_user = "UPDATE users SET name = '$name', surname = '$surname', job = '$job', country = '$country', gender = '$gender', age = '$age', live_place = '$l_p', marital_status = '$m_p' WHERE id_user = $id_user";

					if(!$execute_query = $connect->query($query_user))
					{
						throw new Exception($connect->error);
						$_SESSION['no_settings'] = 'wystąpił błąd :[';
					}
				}	
			}
		}
...
?>			
					<div class="col-10 col-md-9 col-lg-7 formularz_log mx-auto mt-3 p-3 pt-4 text-center">
