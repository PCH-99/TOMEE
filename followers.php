...
if(!isset($_POST['followers']) || !isset($_SESSION['logged'])) 
{
			...
			else
			{
				...
				if(check_input($who) == true)
				{
					...
					else
					{
						...
						$query_user = "SELECT * FROM observations WHERE id_watched = '$id_who'";
						...
						else
						{
							 ...
							 if($ile_wynikow == 0)
							 {
								 if($id_who == $id_user)
								 {
									 $opis = '<div class="col-8 m-4 username text-center mx-auto py-2"> Nikt jeszcze nie obserwuje Ciebie. (ಥ﹏ಥ) </div>';
								 }
								 else
								 {
									 $opis = '<div class="col-8 m-4 username text-center mx-auto py-2"> Nikt jeszcze nie obserwuje tego użytkownika. (ಥ﹏ಥ) </div>';
								 }								 
							 }
							 else if($ile_wynikow != 0)
							 {	
								$ok = true;
								$opis = '<div class="col-8 m-4 username text-center mx-auto py-2"> Obserwujący: </div>';
							 }
						}
					}
				}
